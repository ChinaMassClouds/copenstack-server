...
from openstack_dashboard.dashboards.project.applyhost \
...
from openstack_dashboard.dashboards.project.applyhost import workflows as project_workflows
...
class IndexView(tables.DataTableView):
    ...
    template_name = 'project/applyhost/index.html'

    def filter_data(self, data):
        filters = self.get_filters()
        for key in filters.keys():
            if key == 'name':
                data = data.filter(name__contains=filters[key])
            if key == 'status':
                data = data.filter(status=filters[key])
            if key == 'image_id':
                data = data.filter(image_id=filters[key])
            if key == 'flavor_id':
                data = data.filter(flavor_id=filters[key])
        return data

    def get_data(self):
        """
        achieve instance status !=5 data
        """
        instances = apply.objects.filter(user_name=self.request.user.username).exclude(status='5')
        instances = self.filter_data(instances)
        try:
    ...
    def reverse_translate_status(self, status_name):
        dic = project_tables.APPLY_DISPLAY_CHOICES_TRANSLATE

        for i in dic:
            if ugettext(i[1]) == status_name:
                return i[0]
        return ''
    ...
    def get_filters(self):
        ...
        if filter_field == 'status':
            filter_string = self.reverse_translate_status(filter_string)
...
class UpdateView(workflows.WorkflowView):
    ...
    success_url = reverse_lazy("horizon:project:applyhost:index")
    ...
    def get_object(self, *args, **kwargs):
        ...
        try:
            ...
            redirect = reverse("horizon:project:applyhost:index")
...
class RebuildView(forms.ModalFormView):
    ...
    template_name = 'project/applyhost/rebuild.html'
    success_url = reverse_lazy('horizon:project:applyhost:index')
...
class DecryptPasswordView(forms.ModalFormView):
    ...
    template_name = 'project/applyhost/decryptpassword.html'
    success_url = reverse_lazy('horizon:project:applyhost:index')
...
class DetailView(tabs.TabView):
    ...
    template_name = 'project/applyhost/detail.html'
    redirect_url = 'horizon:project:applyhost:index'
