...
class SetInstanceDetailsAction(workflows.Action):
    ...
    class Meta:
        name = _("Details")
        help_text_template = ("project/applyhost/"
    ...
    def set_meta_data(self, cleaned_data):
        ...
        if availability_zone_info_str:
            ...
            if availability_zone_info and availability_zone_info.get(availability_zone) \
                ...
                platformtype = availability_zone_info.get(availability_zone).get('vtype')
...
class ReasonAction(workflows.Action):
    class Meta:
        name = _("Reason")
        help_text_template = ("project/applyhost/"
                              "_launch_reason_help.html")
    ...
    def __init__(self, *args):
        super(ReasonAction, self).__init__(*args)



class ReasonStep(workflows.Step):
    action_class = ReasonAction
    contributes = ('info',)
...
class SetNetwork(workflows.Step):
    ...
    if api.neutron.is_port_profiles_supported():
        ...
        template_name = "project/applyhost/_update_networks.html"
...
class LaunchInstance(workflows.Workflow):
    slug = "apply_host"
    name = _("Apply Host")
    finalize_button_name = _("Apply")
    success_message = _('Applyed %(count)s named "%(name)s".')
    failure_message = _('Unable to apply %(count)s named "%(name)s".')
    success_url = "horizon:project:applyhost:index"
    multipart = True
    ...
    def handle(self, request, context):
        ...
        try:
            """
             instance data store mysql
            """
            apply_res = apply(user_name=request.user.username,
                              project_name=request.user.tenant_name,
                              name=context['name'],
                              image_id=image_id,
                              flavor_id=context['flavor'],
                              keypair_id=context['keypair_id'],
                              custom_script=custom_script,
                              security_group_ids=context['security_group_ids'],
                              dev_mapping_1=dev_mapping_1,
                              ...
                              meta=json.dumps( context.get('metadata')),
                              info=info,
                              status='0')
            apply_res.save()
            operate_log(request.user.username,
                        request.user.roles,
                        str(context['name'])+" instance apply")
