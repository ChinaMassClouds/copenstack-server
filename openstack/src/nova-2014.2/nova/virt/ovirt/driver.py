...
import utils
from cservermanager import CServerManager
import common
...
class OvirtDriver(driver.ComputeDriver):
    ...
    def __init__(self, virtapi, read_only=False):
        ...
        host_ip, host_username, host_password = utils.get_cserver_host_info()
        if not host_ip or not host_username or not host_password:
            raise Exception(_("Must specify host_ip, host_username and "
                              "host_password to use ovirt.OvirtDriver"))
            
        self._cserver_manager = CServerManager(host_ip, host_username, host_password)
    ...
    def _connect(uri, read_only):
        ...
        try:
            ...
            payload = dict(ip=OvirtDriver.get_host_ip_addr(),
                           method='_connect',
                           reason=ex)
...
#         self._destroy(instance)
        ...
        cserver_uuid = self._cserver_manager.get_cserver_uuid_map(instance.uuid)
        self._cserver_manager.delete_vm(cserver_uuid)
    ...
    def _cleanup_rbd(self, instance):
        OvirtDriver._get_rbd_driver().cleanup_volumes(instance)
...
#                 soft_reboot_success = False
            ...
            cserver_uuid = self._cserver_manager.get_cserver_uuid_map(instance.uuid)
            soft_reboot_success = self._cserver_manager.control_vm_action(cserver_uuid, common.CSERVER_VM_REBOOT)
    ...
    def _hard_reboot(self, context, instance, network_info,
                     block_device_info=None):
        cserver_uuid = self._cserver_manager.get_cserver_uuid_map(instance.uuid)
        self._cserver_manager.control_vm_action(cserver_uuid, common.CSERVER_VM_STOP)
        for i in range(60):
            if self._cserver_manager.get_vm_power_state(cserver_uuid) == 4:
                self._cserver_manager.control_vm_action(cserver_uuid, common.CSERVER_VM_START)
                break
            time.sleep(1)
        ...
        if soft_reboot_success:
            LOG.info(_LI("Instance hard rebooted successfully."),
                     instance=instance)
            return
        else:
            LOG.warn(_LW("Failed to hard reboot instance. "
                         "Trying hard reboot."),
                     instance=instance)
                
    def _hard_reboot_(self, context, instance, network_info,
                     block_device_info=None):
        """Reboot a virtual machine, given an instance reference.
...
#         dom.resume()
        ...
        cserver_uuid = self._cserver_manager.get_cserver_uuid_map(instance.uuid)
        self._cserver_manager.control_vm_action(cserver_uuid, "start")
        ...
        def _wait_for_on():
            ...
            if state == power_state.RUNNING:
                LOG.info(_LI("Instance start successfully."),
                         instance=instance)
                raise loopingcall.LoopingCallDone()

        timer = loopingcall.FixedIntervalLoopingCall(_wait_for_on)
        timer.start(interval=0.5).wait()
...
#         self._destroy(instance)
        ...
        cserver_uuid = self._cserver_manager.get_cserver_uuid_map(instance.uuid)
        self._cserver_manager.control_vm_action(cserver_uuid, common.CSERVER_VM_SHUTDOWN)
        
        def _wait_for_off():
            """Called at an interval until the VM is shutdown."""
            state = self.get_info(instance)['state']

            if state == power_state.SHUTDOWN:
                LOG.info(_LI("Instance stop successfully."),
                         instance=instance)
                raise loopingcall.LoopingCallDone()

        timer = loopingcall.FixedIntervalLoopingCall(_wait_for_off)
        timer.start(interval=0.5).wait()
...
#         dom.managedSave(0)
        ...
        cserver_uuid = self._cserver_manager.get_cserver_uuid_map(instance.uuid)
        self._cserver_manager.control_vm_action(cserver_uuid, "suspend")
        ...
        def _wait_for_off():
            ...
            if state == power_state.SUSPENDED:
                LOG.info(_LI("Instance suspend successfully."),
                         instance=instance)
                raise loopingcall.LoopingCallDone()
...
#         LOG.debug("Instance is running", instance=instance)
        ...
        if instance.metadata.has_key("cluster_id"):
            cluster_id = instance.metadata["cluster_id"]

        if instance.metadata.has_key("template_id"):
            template_id = instance.metadata["template_id"]
   
        if instance.metadata.has_key("flavor_id"):
            flavor_id = instance.metadata["flavor_id"]
        ...
        cserver_vm_uuid = self._cserver_manager.spawn(vm_info)
        
        def _wait_for_boot():
            """Called at an interval until the VM is running."""
#             LOG.info("waiting the state..............................")
            state = self.get_info(instance)['state']
...
#         host = CONF.vncserver_proxyclient_address
        ...
        cserver_uuid = self._cserver_manager.get_cserver_uuid_map(instance.uuid)
        self._cserver_manager.set_vm_ticket(cserver_uuid)
        host, port = self._cserver_manager.get_vm_vnc_host_port(cserver_uuid)   
...
#             raise exception.NovaException(msg)
# 
#         LOG.info("get info...........................: %s" % dom_info)
        cserver_uuid = self._cserver_manager.get_cserver_uuid_map(instance.uuid)
        power_state = self._cserver_manager.get_vm_power_state(cserver_uuid)
        memory_size = self._cserver_manager.get_total_memory()
    ...
    def _get_local_gb_info():
        ...
        if CONF.libvirt.images_type == 'lvm':
            ...
            info = OvirtDriver._get_rbd_driver().get_pool_info()
        else:
    ...
    def get_diagnostics(self, instance):
        ...
        # get io status
        xml = domain.XMLDesc(0)
        dom_io = OvirtDriver._get_io_devices(xml)
...
#         data["memory_mb"] = self.driver._get_memory_mb_total()
        data["memory_mb"] = self._cserver_manager.get_total_memory()
