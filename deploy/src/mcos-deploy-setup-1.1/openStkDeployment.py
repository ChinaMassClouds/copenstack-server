...
def deployOpenstack():
	...
	for opstkComponentNum in range(string.atoi(conf.get("componentorder","componentnum"))):
		...
		logger.info("====================================================")
		nodenum=conf.get("componentinfo",opstkCom+"num")
		for i in range(string.atoi(nodenum)):
			nodeName=conf.get("componentinfo",opstkCom+"node"+str(i))
			if lastDeployStatus=="success" and not nodeName in list:
				continue
			#print("deploying %s on %s" %(opstkCom, nodeName))
			logger.info("deploy %s on %s" %(opstkCom, nodeName))
			...
			if opstkCom == "nfs":
				if conf.get("storagetype","vminstance") == "local" or conf.get("componentinfo","controller-novanode0") == conf.get("componentinfo","computernode0"):
					continue
				else:
					command = "setup-store-nfs"
			if opstkCom == "controller-glance":
				command = "setup-controller-glance"
	
			if opstkCom == "controller-nova":
				if conf.get("componentinfo","controller-novanode0") == conf.get("componentinfo","computernode0"):
			...
			if opstkCom == "controller-neutron":
				if conf.get("componentinfo","controller-novanode0") == conf.get("componentinfo","neutronnode0"):
					...
					opstkCom = "controller-neutron"
					command = "setup-controller-neutron"
			if opstkCom == "neutron":
				if conf.get("componentinfo","controller-novanode0") == conf.get("componentinfo","neutronnode0"):
					continue
				else:
			...
			if opstkCom == "cinder":
				if conf.get("componentinfo","controller-novanode0") == conf.get("componentinfo","cindernode0"):
					command = "setup-store-cinder /dev/sda2"
				else:
					command = "setup-store-cinder"

			if opstkCom == "computer":
				if not conf.get("componentinfo","controller-novanode0") == conf.get("componentinfo","computernode0"):
                                        opstkCom = getNeutronScriptToComputer()
                                        if opstkCom == "computer-gre":
                                                command = "setup-compute-gre-node"
                                        if opstkCom == "computer-vlan":
			...
			#print ("##**##&start:"+opstkCom+":"+nodeName)
			logger.info("##**##&start:"+opstkCom+":"+nodeName)
